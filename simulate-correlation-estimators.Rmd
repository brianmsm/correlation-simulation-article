...
## Generate plot about p-value

## Plot with complete conditions
...
ggsave("img/Evaluate_power_correlation_A.png",
       plot = plot_power_correlation_A,
       height = 7,
       width = 14,
       dpi = 400)
```
...
knitr::include_graphics("img/Evaluate_power_correlation_A.png",
                        dpi = 400)
```

## Plot with summarised conditions
...
```{r eval=FALSE, include=FALSE}
ggsave("img/Evaluate_power_correlation_B.png",
       plot = plot_power_correlation_B,
       height = 7,
       width = 12,
       dpi = 400)
```
...
```{r echo=FALSE, out.width='100%'}
knitr::include_graphics("img/Evaluate_power_correlation_B.png",
                        dpi = 400)
```
...
```{r}
plot_assess_C <- df_work_tidy_simp %>%
  mutate(
    correlacion = factor(correlacion, 
                         labels = c("0.12", "0.20",
                                    "0.31", "0.50")),
    Tipo_Sim = fct_collapse(Tipo_Sim,
                            ...
                            "No-MVN" = c("No-MVN A", "No-MVN C", "No-MVN D"),
                            "No-MVN ks+" = c("No-MVN B", "No-MVN E")),
    Ajuste = str_replace(Ajuste, "RMSE", "ARMSE"),
    Ajuste = str_replace(Ajuste, "Sesgo", "Bias")
  ) %>%  
  ggplot(aes(x = Tipo_Sim, y = Valor,
             # colour = Ajuste,
             shape = Ajuste,
...
# ggsave(filename = "img/Evaluate_correlation_RMSEA_Bias_C.png",
#        plot = plot_assess_C, width = 10.75, height = 7, 
#        dpi = 300)
```

```{r echo=FALSE, out.width='100%'}
knitr::include_graphics("img/Evaluate_correlation_RMSEA_Bias_C.png")
```
